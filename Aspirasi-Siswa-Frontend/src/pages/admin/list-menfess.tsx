...
export default function ListMenfess() {
    ...
    const [items, setItems] = useState<MenfessItem[]>([])
    ...
    useEffect(() => {
        ...
        const loadMenfess = async () => {
            try {
                ...
                const response = await fetchWithAdminAuth(`/api/v1/admin/menfess?${params.toString()}`, {
                    method: "GET",
                    signal: controller.signal,
                })

                const payload: MenfessListResponse | null = await response.json().catch(() => null)
                ...
                if (!response.ok) {
                    ...
                    throw new Error(payload?.message ?? "Gagal mengambil data menfess")
                }
    ...
    const fetchRenderedBlob = async (id: number): Promise<Blob> => {
        const generateResponse = await fetchWithAdminAuth(`/api/v1/admin/menfess/image/${id}`, {
            method: "GET",
        })
        ...
        if (!generateResponse.ok) {
            throw new Error(generatePayload?.message ?? "Gagal generate gambar menfess")
        }
    ...
    const handleDownload = async (id: number) => {
        try {
            ...
            triggerDownload(blob, `menfess-${id}.png`)
            showToast("success", "Gambar menfess berhasil diunduh")
        } catch (error) {
            if (error instanceof Error) {
    ...
    const handleShare = async (id: number) => {
        try {
            ...
            const blob = await fetchRenderedBlob(id)
            const file = new File([blob], `menfess-${id}.png`, { type: blob.type || "image/png" })

            if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
                await navigator.share({
                    title: "Menfess",
                    text: "Bagikan hasil render menfess",
                    files: [file],
                })
    ...
    const confirmDeleteSelected = async () => {
        ...
        try {
            ...
            const response = await fetchWithAdminAuth(`/api/v1/admin/menfess/bulk-delete`, {
                method: "POST",
                headers: {
            ...
            setSelectedIds([])
            showToast("success", "Data menfess terpilih berhasil dihapus")
    ...
    return (
        <div className="mt-10 w-full md:w-3xl">
            ...
            {!loading && !errorMessage && items.length === 0 && (
                <div className="mt-10 rounded-2xl bg-white/20 px-6 py-8 text-center text-white backdrop-blur-md">
                    Belum ada menfess yang cocok dengan filter.
                </div>
            )}
            ...
            <ConfirmDeleteModal
                ...
                message={`Hapus ${selectedIds.length} data menfess terpilih? Tindakan ini tidak bisa dibatalkan.`}
                loading={deleting}
