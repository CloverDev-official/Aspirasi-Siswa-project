...
export default function Menfess() {
  ...
  useEffect(() => {
    ...
    const loadSchedule = async () => {
      try {
        const data = await fetchScheduleStatus("menfess")
        if (active) {
          setSchedule(data)
        }
      } finally {
  ...
  const sendToApi = async () => {
    try {
      ...
      const res = await fetch("/api/v1/menfess", {
        method: "POST",
        headers: {
      ...
      const payload: MenfessApiPayload | null = await res.json().catch(() => null)

      if (!res.ok) {
        if (payload?.data?.status) {
          setSchedule(payload.data.status)
        }
        const errorMessage = payload?.message ?? "Gagal mengirim menfess"
  ...
  if (isClosed && schedule) {
    return (
      <div className="mt-14 w-full md:w-3xl">
        <ScheduleClosedCard title="Menfess" scheduleText={schedule.schedule_text} nextOpenAt={schedule.next_open_at} />
      </div>
    )
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4 py-6 overflow-y-auto">
          <div className="w-full max-w-md rounded-2xl bg-linear-to-b from-[#7A1E2D] via-[#B23A48] to-[#FBE49D] p-5 sm:p-6 text-center shadow-[0_22px_50px_rgba(0,0,0,0.38)]">
            <div className="flex flex-col items-center">
                ...
                <img src="/gambar-modal-konfirmasi.png" alt="konfirmasi menfess" className="w-40 sm:w-44 h-auto" />
                    </div>
